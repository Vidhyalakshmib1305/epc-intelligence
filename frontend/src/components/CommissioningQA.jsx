  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      ...
      <div style={{ position:'absolute', width:400, height:400, borderRadius:'50%',
        background:'radial-gradient(circle,rgba(244,114,182,0.13) 0%,transparent 70%)',
...
const STATUS_CFG = {
  'PASS':    { grad:'from-green-500 to-emerald-700', glow:'glow-yellow', icon:'✅', critical:false },
  'FAIL':    { grad:'from-red-600 to-red-900',       glow:'glow-red',    icon:'❌', critical:true  },
  'PARTIAL': { grad:'from-amber-500 to-orange-600',  glow:'glow-orange', icon:'⚠️', critical:false },
}

const DOTS = ['⠋','⠙','⠹','⠸','⠼','⠴','⠦','⠧','⠇','⠏']

export default function CommissioningQA() {
  ...
  const run = async (e) => {
    ...
    try {
      await streamAgent(
        '/api/agents/commissioning-qa/stream',
        { question: query, top_k: topK },
        tok => setStream(tok),
        (analysis, meta, sources) => {
          setResult({ analysis, status: meta?.test_status, sources: sources || [], rewrittenQuery: meta?.rewritten_query, retrievalScore: meta?.retrieval_score })
          setLoading(false)
  ...
  const cfg = result ? (STATUS_CFG[result.status] || STATUS_CFG['PARTIAL']) : null

  return (
    <div className="slide-in relative min-h-screen -mx-8 -mt-8 px-8 pt-8 pb-14"
      style={{ background:'linear-gradient(135deg,#1a0820 0%,#2d0a35 40%,#3a0d42 70%,#1a0820 100%)' }}>
      <Aurora />
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-3"
            ...
            <span className="text-pink-300 text-xs font-bold tracking-widest uppercase">Commissioning QA Agent</span>
          </div>
          <h1 className="text-3xl font-black text-white">Commissioning QA Monitor</h1>
          <p className="text-slate-400 text-sm mt-1">Track NCRs, test results and Tier III readiness across all commissioning phases</p>
        </div>
        ...
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="col-span-2">
            <div className="rounded-2xl p-6 sticky top-6 transition-all duration-300"
              style={{
                ...
                border:`1px solid ${focused ? 'rgba(236,72,153,0.5)' : 'rgba(255,255,255,0.08)'}`,
                boxShadow: focused ? '0 0 40px rgba(236,72,153,0.12)' : 'none',
              }}>
              <form onSubmit={run} className="space-y-5">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Commissioning Query</label>
                  <textarea rows={6} value={query} onChange={e => setQuery(e.target.value)}
                    onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
                    placeholder="e.g. What NCRs are currently open? Did UPS load tests pass? What is the Tier III readiness status?"
                ...
                <div>
                  <label className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                    Top K Sources: <span className="text-pink-400 font-black">{topK}</span>
                  </label>
                  <input type="range" min={1} max={8} value={topK}
                    onChange={e => setTopK(+e.target.value)} className="w-full mt-1.5 accent-pink-500"/>
                </div>
                <button type="submit" disabled={loading || !query.trim()}
                  ...
                  style={{
                    background: loading ? 'rgba(236,72,153,0.4)' : 'linear-gradient(135deg,#ec4899,#db2777)',
                    boxShadow: loading ? 'none' : '0 4px 24px rgba(236,72,153,0.38)',
                    transform: loading ? 'scale(0.98)' : 'scale(1)',
                  }}>
                  {loading ? `${DOTS[dotIdx]} Analysing…` : '🔬 Check Commissioning'}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="col-span-3 space-y-4">
            {!loading && !result && !error && (
              <Idle3D icon="🔬" color="#ec4899" label="Query commissioning data to see test status and NCRs" />
            )}

            {loading && (
              <div className="rounded-2xl p-8 flex flex-col items-center gap-5"
                style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(236,72,153,0.18)' }}>
                <div className="relative w-20 h-20">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="absolute inset-0 rounded-full border-2 border-pink-500"
                      style={{ animation:`radar 1.8s ${i * 0.6}s ease-out infinite` }}/>
                  ))}
                  <div className="absolute inset-0 flex items-center justify-center text-3xl">🔬</div>
                </div>
                <div className="text-center">
                  <p className="text-pink-300 font-bold">Scanning Commissioning Records…</p>
              ...
              <div className={revealed ? 'flip-in-3d space-y-4' : 'opacity-0'}>
                  ...
                  <div className={`pop-bounce bg-gradient-to-r ${cfg.grad} ${cfg.glow} rounded-2xl p-5 text-white relative z-10`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-xs opacity-70 uppercase tracking-widest font-semibold">Test Status</p>
                ...
                <div className="rounded-2xl overflow-hidden"
                  style={{ background:'rgba(4,8,20,0.85)', backdropFilter:'blur(20px)',
                    border:'1px solid rgba(236,72,153,0.28)', boxShadow:'0 0 50px rgba(236,72,153,0.07)' }}>
                  <div className="flex items-center gap-2 px-5 py-3"
                    style={{ background:'rgba(255,255,255,0.04)', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
                    <div className="w-3 h-3 rounded-full bg-red-500/80"/>
                    <div className="w-3 h-3 rounded-full bg-yellow-400/80"/>
                    <div className="w-3 h-3 rounded-full bg-green-500/80"/>
                    <span className="ml-3 text-xs text-slate-400 font-mono">commissioning-qa › analysis</span>
                    <span className="ml-auto text-xs text-pink-400 font-mono">✓ complete</span>
                ...
                {result.rewrittenQuery && result.rewrittenQuery !== query && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
                    ...
                    <span className="text-xs text-pink-300 font-mono italic truncate">{result.rewrittenQuery}</span>
                  </div>
                )}
                      ...
                      <div key={i} className="slide-from-right rounded-xl p-4"
                        ...
                        <div className="flex items-start gap-3">
                          <span className="flex-none w-6 h-6 rounded-md flex items-center justify-center text-xs font-black text-white"
                            style={{ background:'linear-gradient(135deg,#ec4899,#db2777)' }}>{i + 1}</span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className="px-2 py-0.5 rounded text-xs font-bold text-pink-300"
                            ...
                            {src.text_preview && (
                              <p className="text-slate-400 text-xs leading-relaxed italic border-l-2 border-pink-900 pl-2">
