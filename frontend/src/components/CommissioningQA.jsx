...
export default function CommissioningQA() {
  ...
  const handleAnalyse = async () => {
    ...
    try {
      ...
      setResult({ analysis: '', sources: [], test_status: null })
      await streamAgent(
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-800 mb-1">Commissioning QA</h2>
      <p className="text-gray-500 mb-6">Check test results and open non-conformances against TIA-942 and Tier III standards.</p>
      <div className="grid grid-cols-10 gap-4">
        <div className="col-span-3">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">QA Query</label>
              <textarea value={question} onChange={e => setQuestion(e.target.value)}
                placeholder="e.g. What are the open non-conformances blocking Tier III certification?"
                rows={5}
            ...
            <div>
              ...
              <input type="range" min={1} max={10} value={topK}
                onChange={e => setTopK(Number(e.target.value))} className="w-full" />
            </div>
            <button onClick={handleAnalyse} disabled={!question.trim() || loading || streaming}
              className="w-full bg-blue-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
              {loading ? 'Searching...' : streaming ? 'Analysing...' : 'Check QA Status'}
            </button>
          </div>
        </div>
        ...
        <div className="col-span-4 space-y-3">
          ...
          {result?.test_status && (
            <div className={`rounded-xl border-2 p-4 ${statusColors[result.test_status] || 'bg-gray-100 text-gray-800 border-gray-300'}`}>
              <p className="text-xs opacity-70 mb-1">Test Status</p>
              <p className="text-2xl font-bold">{result.test_status}</p>
            </div>
          )}
          {result && (
        ...
        <div className="col-span-3">
          {result?.sources?.length > 0 && (
            <div className="bg-gray-50 rounded-xl border border-gray-200 p-4">
              <h3 className="font-semibold text-gray-700 mb-3 text-sm">QA References</h3>
