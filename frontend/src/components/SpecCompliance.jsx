...
export default function SpecCompliance() {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      setResult({ analysis: '', sources: [], compliance_status: null })
      await streamAgent(
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 space-y-4">
            <div>
              ...
              <textarea value={question} onChange={e => setQuestion(e.target.value)}
                placeholder="e.g. Is the proposed UPS with 94% efficiency compliant?" rows={5}
            ...
            <button onClick={handleCheck} disabled={!question.trim() || loading || streaming}
              className="w-full bg-blue-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
              {loading ? 'Searching...' : streaming ? 'Analysing...' : 'Check Compliance'}
            </button>
          </div>
        </div>
