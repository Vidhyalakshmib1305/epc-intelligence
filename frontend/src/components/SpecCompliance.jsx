...
export default function SpecCompliance() {
  ...
  return (
    <div className="max-w-3xl">
      <h2 className="text-2xl font-bold text-gray-800 mb-2">Spec Compliance</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
        <div>
          ...
          <textarea value={question} onChange={e => setQuestion(e.target.value)}
            placeholder="e.g. Is the proposed UPS with 94% efficiency compliant with the spec?" rows={3}
      ...
      {result && (
        <div className="mt-4 space-y-4">
          <div className={`rounded-xl border-2 p-4 ${statusColors[result.compliance_status] || 'bg-gray-100 text-gray-800 border-gray-300'}`}>
            <p className="text-sm opacity-70">Compliance Status</p>
          ...
          {result.sources?.length > 0 && (
            <div className="bg-gray-50 rounded-xl border border-gray-200 p-6">
              <h3 className="font-semibold text-gray-700 mb-3">Spec References</h3>
