    ...
    try {
      await streamAgent(
        ...
        (analysis, meta, sources) => {
          setResult({ analysis, status: meta?.compliance_status, sources: sources || [], rewrittenQuery: meta?.rewritten_query, retrievalScore: meta?.retrieval_score })
          setLoading(false)
              ...
              <div className={revealed ? 'flip-in-3d space-y-4' : 'opacity-0'}>
                ...
                <div className="rounded-2xl overflow-hidden"
                  style={{ background:'rgba(4,8,20,0.85)', backdropFilter:'blur(20px)',
                    border:'1px solid rgba(59,130,246,0.28)', boxShadow:'0 0 50px rgba(59,130,246,0.07)' }}>
                  <div className="flex items-center gap-2 px-5 py-3"
                    style={{ background:'rgba(255,255,255,0.04)', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
                    <div className="w-3 h-3 rounded-full bg-red-500/80"/>
                    <div className="w-3 h-3 rounded-full bg-yellow-400/80"/>
                    <div className="w-3 h-3 rounded-full bg-green-500/80"/>
                    <span className="ml-3 text-xs text-slate-400 font-mono">spec-compliance › analysis</span>
                    <span className="ml-auto text-xs text-blue-400 font-mono">✓ complete</span>
                ...
                {result.rewrittenQuery && result.rewrittenQuery !== query && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
                    ...
                    <span className="text-xs text-blue-300 font-mono italic truncate">{result.rewrittenQuery}</span>
                  </div>
                )}
                {/* Sources */}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className="px-2 py-0.5 rounded text-xs font-bold text-blue-300"
                            ...
                            {src.text_preview && (
                              <p className="text-slate-400 text-xs leading-relaxed italic border-l-2 border-blue-800 pl-2">
