...
const STATUS_CFG = {
  'HIGH':   { grad:'from-red-600 to-red-900',       glow:'glow-red',    icon:'🔴', critical:true  },
  'MEDIUM': { grad:'from-amber-500 to-orange-700',  glow:'glow-orange', icon:'🟡', critical:false },
  'LOW':    { grad:'from-green-500 to-emerald-700', glow:'glow-yellow', icon:'🟢', critical:false },
}
...
export default function ScheduleRisk() {
    ...
    try {
      await streamAgent(
        ...
        (analysis, meta, sources) => {
          setResult({ analysis, status: meta?.risk_level, sources: sources || [], rewrittenQuery: meta?.rewritten_query, retrievalScore: meta?.retrieval_score })
          setLoading(false)
  ...
  return (
    <div className="slide-in relative min-h-screen -mx-8 -mt-8 px-8 pt-8 pb-14"
      style={{ background:'linear-gradient(135deg,#0c0720 0%,#140d2e 40%,#1a1035 70%,#0c0720 100%)' }}>
      <Aurora />
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-3"
            ...
            <span className="text-violet-300 text-xs font-bold tracking-widest uppercase">Schedule Risk Agent</span>
          </div>
          <h1 className="text-3xl font-black text-white">Schedule Risk Analysis</h1>
          <p className="text-slate-400 text-sm mt-1">Predict delays and identify critical path threats before they escalate</p>
        </div>
        ...
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="col-span-2">
            <div className="rounded-2xl p-6 sticky top-6 transition-all duration-300"
              style={{
                ...
                boxShadow: focused ? '0 0 40px rgba(139,92,246,0.12)' : 'none',
              }}>
              <form onSubmit={run} className="space-y-5">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Risk Query</label>
                  <textarea rows={6} value={query} onChange={e => setQuery(e.target.value)}
                    onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
                    placeholder="e.g. Which tasks are most at risk of delaying the critical path?"
                ...
                <div>
                  <label className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                    Top K Sources: <span className="text-violet-400 font-black">{topK}</span>
                  </label>
                  <input type="range" min={1} max={8} value={topK}
                    onChange={e => setTopK(+e.target.value)} className="w-full mt-1.5 accent-violet-500"/>
                </div>
                <button type="submit" disabled={loading || !query.trim()}
                  ...
                  style={{
                    ...
                    boxShadow: loading ? 'none' : '0 4px 24px rgba(139,92,246,0.38)',
                    transform: loading ? 'scale(0.98)' : 'scale(1)',
                  }}>
                  {loading ? `${DOTS[dotIdx]} Analysing…` : '📅 Analyse Risk'}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="col-span-3 space-y-4">
            {!loading && !result && !error && (
              <Idle3D icon="📅" color="#8b5cf6" label="Enter your query to begin risk analysis" />
            )}
            ...
            {loading && (
              <div className="rounded-2xl p-8 flex flex-col items-center gap-5"
                ...
                <div className="relative w-20 h-20">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="absolute inset-0 rounded-full border-2 border-violet-500"
                      style={{ animation:`radar 1.8s ${i * 0.6}s ease-out infinite` }}/>
                  ))}
                  <div className="absolute inset-0 flex items-center justify-center text-3xl">📅</div>
                </div>
                <div className="text-center">
                  <p className="text-violet-300 font-bold">Scanning Schedule Documents…</p>
              ...
              <div className={revealed ? 'flip-in-3d space-y-4' : 'opacity-0'}>
                  ...
                  <div className={`pop-bounce bg-gradient-to-r ${cfg.grad} ${cfg.glow} rounded-2xl p-5 text-white relative z-10`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-xs opacity-70 uppercase tracking-widest font-semibold">Risk Level</p>
                ...
                <div className="rounded-2xl overflow-hidden"
                  style={{ background:'rgba(4,8,20,0.85)', backdropFilter:'blur(20px)',
                    border:'1px solid rgba(139,92,246,0.28)', boxShadow:'0 0 50px rgba(139,92,246,0.07)' }}>
                  <div className="flex items-center gap-2 px-5 py-3"
                    style={{ background:'rgba(255,255,255,0.04)', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
                    <div className="w-3 h-3 rounded-full bg-red-500/80"/>
                    <div className="w-3 h-3 rounded-full bg-yellow-400/80"/>
                    <div className="w-3 h-3 rounded-full bg-green-500/80"/>
                    <span className="ml-3 text-xs text-slate-400 font-mono">schedule-risk › analysis</span>
                    <span className="ml-auto text-xs text-violet-400 font-mono">✓ complete</span>
                ...
                {result.rewrittenQuery && result.rewrittenQuery !== query && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
                    ...
                    <span className="text-xs text-violet-300 font-mono italic truncate">{result.rewrittenQuery}</span>
                  </div>
                )}
                      ...
                      <div key={i} className="slide-from-right rounded-xl p-4"
                        ...
                        <div className="flex items-start gap-3">
                          <span className="flex-none w-6 h-6 rounded-md flex items-center justify-center text-xs font-black text-white"
                            style={{ background:'linear-gradient(135deg,#8b5cf6,#6d28d9)' }}>{i + 1}</span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className="px-2 py-0.5 rounded text-xs font-bold text-violet-300"
                            ...
                            {src.text_preview && (
                              <p className="text-slate-400 text-xs leading-relaxed italic border-l-2 border-violet-800 pl-2">
